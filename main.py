...
def start(message):
    ...
    bot.send_message(message.chat.id, "أهلاً بك! تم تفعيل نظام التداول الآلي الذكي مع تحليل RSI و EMA:", reply_markup=markup)

@bot.message_handler(func=lambda m: m.text == "🤖 تفعيل التداول الآلي")
def enable_auto(message):
    uid = message.from_user.id
    auto_trading_users.add(uid)
    bot.send_message(message.chat.id, "✅ **تم تفعيل التداول الآلي الذكي!**", parse_mode="Markdown")
