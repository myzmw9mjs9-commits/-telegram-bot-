...
# --- 1. حل العيب 1: إعداد قاعدة البيانات SQLite لحفظ البيانات بشكل دائم ---
def init_db():
...
# --- 2. حل العيب 4 و 5: طلب البيانات بإطار 5m مع حماية Rate Limits ---
cached_indicators = {'rsi': 50.0, 'ema': 0.0, 'last_update': 0}

def get_klines_analysis():
    now = time.time()
    # تجنب الحظر: تحديث المؤشرات الفنية كل 15 ثانية فقط
...
# --- 3. حلقة الفحص الذكي وإدارة المخاطر والعمولات ---
def auto_market_scanner():
    prev_price = get_live_btc_price()
    FEE = 0.001  # عمولة المنصة 0.1% للشراء و 0.1% للبيع (العيب 3)
            ...
            for uid in auto_users:
                ...
                if not trade and w['usdt'] >= amount:
                    if price_change >= 0.05 and 35 <= rsi <= 60 and current_price >= ema:
                        ...
                        target_tp = current_price * 1.012  # هدف +1.2% لضمان صافي ربح ممتاز بعد العمولات
                        ...
                        msg = (
                            ...
                            f"📈 **السبب:** صعود قوي على فريم (5m) (+{price_change:.2f}%)\n"
                            f"📊 **مؤشر RSI:** {rsi:.1f} (منطقة آمنة)\n"
                            f"💵 **سعر الشراء:** ${current_price:,.2f}\n"
                            f"📉 **العمولة المستقطعة:** $0.10 USDT\n"
                ...
                # 2. حالة التخارج وإدارة الصفقات مع حماية الانزلاق السعري (العيب 2)
                elif trade:
                    # جني الأرباح
                    if current_price >= trade['tp']:
                        gross_returned = trade['btc_bought'] * current_price
                        usdt_returned = gross_returned * (1 - FEE) # خصم عمولة البيع
                        ...
                        msg = (
                            ...
                            f"📈 **صافي الربح بعد العمولات:** +${profit:.2f}\n"
                    ...
                    # وقف الخسارة
                        ...
                        msg = (
                            f"🔴 **تم تفعيل وقف الخسارة لحماية محفظتك!**\n\n"
                            f"💵 **سعر البيع:** ${current_price:,.2f}\n"
                            f"📉 **صافي النتيجة بعد العمولات:** ${loss:.2f}\n"
                            f"💰 **رصيد المحفظة الحالي:** ${w['usdt']:.2f}"
                        )
                        bot.send_message(uid, msg, parse_mode="Markdown")
...
def start(message):
    ...
    bot.send_message(message.chat.id, "أهلاً بك! تم تشغيل البوت المتكامل (قاعدة بيانات + تحليلات 5m + خصم عمولات + حماية السيرفر):", reply_markup=markup)
...
def enable_auto(message):
    ...
    bot.send_message(message.chat.id, "✅ **تم تفعيل التداول الآلي الآمن!**\nالبيانات والصفقات تُحفظ أوتوماتيكياً في قاعدة البيانات.", parse_mode="Markdown")
